...
import sigmorphon_reinflection.dataloader as dataloader
import sigmorphon_reinflection.reinflection_model as model
import sigmorphon_reinflection.util as util
...
class Trainer(object):
    ...
    def smart_load_model(self, model_prefix):
        assert self.model is None
        models = []
        for model in glob.glob(model_prefix + '.nll*'):
    ...
    def decode(self, mode, write_fp, decode_fn=decode_greedy):
        ...
        cnt = 0
        sampler, nb_instance = self.iterate_instance(mode)
        with open(write_fp + '.' + mode + '.guess', 'w') as out_fp, \
             open(write_fp + '.' + mode + '.gold', 'w') as trg_fp:
    ...
    def reload_and_test(self, model_fp, batch_size, best_acc):
        ...
        if best_acc:
            best_fp = best_acc_fp
        self.logger.info('loading ' + best_fp + ' for testing')
        self.load_model(best_fp)
        self.logger.info('decoding dev set')
        self.decode(DEV, model_fp + '.decode')
        if self.data.test_file is not None:
            self.calc_loss(TEST, batch_size)
            self.logger.info('decoding test set')
            self.decode(TEST, model_fp + '.decode')
            results = self.evaluate(TEST)
            results = ' '.join([r.desc + ' ' + r.res for r in results])
            self.logger.info('TEST ' + model_fp.split("/")[-1] + ' ' + results)
        return set([best_fp])
